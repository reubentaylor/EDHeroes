...
export class AdvancementManager extends Application {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["eh5e", "advancement", "flow"],
      template: "systems/eh5e/templates/advancement/advancement-manager.html",
      width: 460,
      height: "auto"
    });
  }
  ...
  get title() {
    ...
    const step = visibleIndex < 0 ? "" : game.i18n.format("EH5E.AdvancementManagerSteps", {
      current: visibleIndex + 1,
      total: visibleSteps.length
    });
    return `${game.i18n.localize("EH5E.AdvancementManagerTitle")} ${step}`;
  }
  ...
  getData() {
    ...
    return {
      ...
      header: item.name,
      subheader: level ? game.i18n.format("EH5E.AdvancementLevelHeader", { level }) : "",
      steps: {
        current: visibleIndex + 1,
        total: visibleSteps.length,
  ...
  render(...args) {
     ...
     * @function eh5e.preAdvancementManagerRender
     * @memberof hookEvents
     * @param {AdvancementManager} advancementManager The advancement manager about to be rendered
     */
    const allowed = Hooks.call("eh5e.preAdvancementManagerRender", this);
  ...
  async close(options={}) {
    if ( !options.skipConfirmation ) {
      return new Dialog({
        title: `${game.i18n.localize("EH5E.AdvancementManagerCloseTitle")}: ${this.actor.name}`,
        content: game.i18n.localize("EH5E.AdvancementManagerCloseMessage"),
        buttons: {
          close: {
            icon: '<i class="fas fa-times"></i>',
            label: game.i18n.localize("EH5E.AdvancementManagerCloseButtonStop"),
            callback: () => super.close(options)
          },
          continue: {
            icon: '<i class="fas fa-chevron-right"></i>',
            label: game.i18n.localize("EH5E.AdvancementManagerCloseButtonContinue")
          }
        },
        default: "close"
      }).render(true);
  ...
  async _restart(event) {
    const restart = await Dialog.confirm({
      title: game.i18n.localize("EH5E.AdvancementManagerRestartConfirmTitle"),
      content: game.i18n.localize("EH5E.AdvancementManagerRestartConfirm")
    });
    if ( !restart ) return;
  ...
  async _complete(event) {
     ...
     * are applied.
     * @function eh5e.preAdvancementManagerComplete
     * @memberof hookEvents
     ...
     */
    if ( Hooks.call("eh5e.preAdvancementManagerComplete", this, updates, toCreate, toUpdate, toDelete) === false ) {
     ...
     * @function eh5e.advancementManagerComplete
     * @memberof hookEvents
     * @param {AdvancementManager} advancementManager The advancement manager that just completed
     */
    Hooks.callAll("eh5e.advancementManagerComplete", this);

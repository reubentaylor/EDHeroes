...
export class ItemGrantAdvancement extends Advancement {
  ...
  static get metadata() {
    return foundry.utils.mergeObject(super.metadata, {
      ...
      order: 40,
      icon: "systems/eh5e/icons/svg/item-grant.svg",
      title: game.i18n.localize("EH5E.AdvancementItemGrantTitle"),
      hint: game.i18n.localize("EH5E.AdvancementItemGrantHint"),
      apps: {
  ...
  summaryForLevel(level, { configMode=false }={}) {
    ...
    if ( !this.data.value.added || configMode ) {
      return this.data.configuration.items.reduce((html, uuid) => html + game.eh5e.utils._linkForUuid(uuid), "");
    }
  ...
  async apply(level, data, retainedData={}) {
    ...
    for ( const [uuid, selected] of Object.entries(data) ) {
      ...
      item.data.update({
        ...
        "flags.eh5e.sourceId": uuid,
        "flags.eh5e.advancementOrigin": `${this.item.id}.${this.id}`
      });
  ...
  restore(level, data) {
    ...
    for ( const item of data.items ) {
      ...
      updates[item._id] = item.flags.eh5e.sourceId;
    }
    this.updateSource({"value.added": updates});
  }
...
export class ItemGrantConfig extends AdvancementConfig {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      ...
      template: "systems/eh5e/templates/advancement/item-grant-config.html"
    });
  }
  ...
  async _onDrop(event) {
    ...
    if ( item.uuid === this.item.uuid ) {
      return ui.notifications.warn(game.i18n.localize("EH5E.AdvancementItemGrantRecursiveWarning"));
    }

    // Abort if this uuid exists already
    if ( existingItems.includes(item.uuid) ) {
      return ui.notifications.warn(game.i18n.localize("EH5E.AdvancementItemGrantDuplicateWarning"));
    }
...
export class ItemGrantFlow extends AdvancementFlow {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      template: "systems/eh5e/templates/advancement/item-grant-flow.html"
    });
  }
  ...
  async getData() {
    ...
    const added = this.retainedData?.items.map(i => foundry.utils.getProperty(i, "flags.eh5e.sourceId"))
  ...
  async _updateObject(event, formData) {
    const retainedData = this.retainedData?.items.reduce((obj, i) => {
      obj[foundry.utils.getProperty(i, "flags.eh5e.sourceId")] = i;
      return obj;
    }, {});

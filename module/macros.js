  ...
  switch ( data.type ) {
    case "Item":
      ...
      foundry.utils.mergeObject(macroData, {
        name: data.data.name,
        img: data.data.img,
        command: `game.eh5e.macros.rollItem("${data.data.name}")`,
        flags: {"eh5e.itemMacro": true}
      });
      break;
    case "ActiveEffect":
      if ( !("data" in data) ) return ui.notifications.warn(game.i18n.localize("MACRO.5eUnownedWarn"));
      foundry.utils.mergeObject(macroData, {
        ...
        img: data.data.icon,
        command: `game.eh5e.macros.toggleEffect("${data.data.label}")`,
        flags: {"eh5e.effectMacro": true}
      });
      break;
    default:
      return;

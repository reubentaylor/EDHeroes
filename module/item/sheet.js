...
export default class ItemSheet5e extends ItemSheet {
  ...
  /** @inheritdoc */
  get template() {
    const path = "systems/eh5e/templates/items/";
  ...
  async _getItemBaseTypes(item) {
    ...
    const ids = CONFIG.EH5E[`${type}Ids`];
  ...
  _getItemConsumptionTargets(item) {
    ...
    // Hit Dice
      ...
      return {
        smallest: game.i18n.localize("EH5E.ConsumeHitDiceSmallest"),
        ...CONFIG.EH5E.hitDieTypes.reduce((obj, hd) => { obj[hd] = hd; return obj; }, {}),
        largest: game.i18n.localize("EH5E.ConsumeHitDiceLargest")
      };
    }
      ...
      return actor.items.reduce((obj, i) => {
        ...
        if ( uses.per && uses.max ) {
          const label = uses.per === "charges"
            ? ` (${game.i18n.format("EH5E.AbilityUseChargesLabel", {value: uses.value})})`
            : ` (${game.i18n.format("EH5E.AbilityUseConsumableLabel", {max: uses.max, per: uses.per})})`;
          obj[i.id] = i.name + label;
        }

        // Recharging items
        const recharge = i.data.data.recharge || {};
        if ( recharge.value ) obj[i.id] = `${i.name} (${game.i18n.format("EH5E.Recharge")})`;
        return obj;
      }, {});
    }
  ...
  _getItemStatus(item) {
    switch ( item.type ) {
      case "class":
        return game.i18n.format("EH5E.LevelCount", {ordinal: item.data.levels.ordinalString()});
      case "equipment":
      case "weapon":
        return game.i18n.localize(item.data.equipped ? "EH5E.Equipped" : "EH5E.Unequipped");
      case "spell":
        return CONFIG.EH5E.spellPreparationModes[item.data.preparation];
      case "tool":
        return game.i18n.localize(item.data.proficient ? "EH5E.Proficient" : "EH5E.NotProficient");
    }
  }
  ...
  _getItemProperties(item) {
    ...
    if ( item.type === "weapon" ) {
      props.push(...Object.entries(item.data.properties)
        .filter(e => e[1] === true)
        .map(e => CONFIG.EH5E.weaponProperties[e[0]]));
    }
      ...
      props.push(CONFIG.EH5E.equipmentTypes[item.data.armor.type]);
    ...
    if ( item.data.actionType ) {
      props.push(CONFIG.EH5E.itemActionTypes[item.data.actionType]);
    }
  ...
  _getSubmitData(updateData={}) {
    ...
    if ( data.data?.uses?.max ) {
      ...
      if ( !maxRoll.isDeterministic ) {
        ...
        return ui.notifications.error(game.i18n.format("EH5E.FormulaCannotContainDiceError", {
          name: game.i18n.localize("EH5E.LimitedUses")
        }));
      }
    }
    ...
    if ( data.data.identifier ) {
      ...
      if ( !match ) {
        ...
        return ui.notifications.error(game.i18n.localize("EH5E.IdentifierError"));
      }
    }
  ...
  _onConfigureTraits(event) {
    ...
    switch (a.dataset.options) {
      case "saves":
        options.choices = CONFIG.EH5E.abilities;
        options.valueKey = null;
        break;
      case "skills.choices":
        options.choices = CONFIG.EH5E.skills;
        options.valueKey = null;
        break;
      case "skills":
        const skills = this.item.data.data.skills;
        const choiceSet = skills.choices?.length ? skills.choices : Object.keys(CONFIG.EH5E.skills);
        options.choices =
          Object.fromEntries(Object.entries(CONFIG.EH5E.skills).filter(([skill]) => choiceSet.includes(skill)));
  ...
  _onAdvancementAction(event) {
    ...
    if ( cl.contains("item-add") ) return game.eh5e.advancement.AdvancementSelection.createDialog(this.item);

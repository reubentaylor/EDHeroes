/**
 * The EH5E game system for Foundry Virtual Tabletop
 ...
 * Repository: https://gitlab.com/foundrynet/eh5e
 * Issue Tracker: https://gitlab.com/foundrynet/eh5e/issues
 */

// Import Modules
import { EH5E } from "./module/config.js";
...
Hooks.once("init", function() {
  console.log(`EH5E | Initializing the EH5E Game System\n${EH5E.ASCII}`);
  ...
  if ( !game.settings.get("eh5e", "honorScore") ) {
    delete EH5E.abilities.hon;
    delete EH5E.abilityAbbreviations.hon;
  }
  if ( !game.settings.get("eh5e", "sanityScore") ) {
    delete EH5E.abilities.san;
    delete EH5E.abilityAbbreviations.san;
  }
  ...
  Actors.registerSheet("eh5e", ActorSheet5eCharacter, {
    types: ["character"],
    makeDefault: true,
    label: "EH5E.SheetClassCharacter"
  });
  Actors.registerSheet("eh5e", ActorSheet5eNPC, {
    types: ["npc"],
    makeDefault: true,
    label: "EH5E.SheetClassNPC"
  });
  Actors.registerSheet("eh5e", ActorSheet5eVehicle, {
    types: ["vehicle"],
    makeDefault: true,
    label: "EH5E.SheetClassVehicle"
  });
  Items.unregisterSheet("core", ItemSheet);
  Items.registerSheet("eh5e", ItemSheet5e, {
    makeDefault: true,
    label: "EH5E.SheetClassItem"
  });
...
Hooks.once("setup", function() {
  CONFIG.EH5E.trackableAttributes = expandAttributeList(CONFIG.EH5E.trackableAttributes);
  CONFIG.EH5E.consumableResources = expandAttributeList(CONFIG.EH5E.consumableResources);
});
 ...
 */
Hooks.once("i18nInit", () => utils.performPreLocalization(CONFIG.EH5E));
...
Hooks.once("ready", function() {
  ...
  const currentVersion = game.settings.get("eh5e", "systemMigrationVersion");
  const NEEDS_MIGRATION_VERSION = "1.6.0";
  const COMPATIBLE_MIGRATION_VERSION = 0.80;
  const totalDocuments = game.actors.size + game.scenes.size + game.items.size;
  if ( !currentVersion && totalDocuments === 0 ) return game.settings.set("eh5e", "systemMigrationVersion", game.system.data.version);
...
Hooks.on("canvasInit", function() {
  ...
  canvas.grid.diagonalRule = game.settings.get("eh5e", "diagonalMovement");
...
Hooks.on("renderChatMessage", (app, html, data) => {
  ...
  if (game.settings.get("eh5e", "autoCollapseItemCards")) html.find(".card-content").hide();
});

...
export default class ActorSheet5eCharacter extends ActorSheet5e {
  ...
  static get defaultOptions() {
    return mergeObject(super.defaultOptions, {
      classes: ["eh5e", "sheet", "actor", "character"]
    });
  }
  ...
  getData() {
    ...
    // Resources
    sheetData.resources = ["genius", "focus", "influence"].reduce((arr, r) => {
      const res = sheetData.data.resources[r] || {};
      res.name = r;
      res.placeholder = game.i18n.localize(`EH5E.Resource${r.titleCase()}`);
    ...
    // Experience Tracking
    sheetData.disableExperience = game.settings.get("eh5e", "disableExperienceTracking");
    ...
    // Weight unit
    sheetData.weightUnit = game.settings.get("eh5e", "metricWeightUnits")
      ? game.i18n.localize("EH5E.AbbreviationKgs")
      : game.i18n.localize("EH5E.AbbreviationLbs");
  ...
  _prepareItems(data) {
    ...
    const inventory = {
      weapon: { label: "EH5E.ItemTypeWeaponPl", items: [], dataset: {type: "weapon"} },
      equipment: { label: "EH5E.ItemTypeEquipmentPl", items: [], dataset: {type: "equipment"} },
      consumable: { label: "EH5E.ItemTypeConsumablePl", items: [], dataset: {type: "consumable"} },
      tool: { label: "EH5E.ItemTypeToolPl", items: [], dataset: {type: "tool"} },
      backpack: { label: "EH5E.ItemTypeContainerPl", items: [], dataset: {type: "backpack"} },
      loot: { label: "EH5E.ItemTypeLootPl", items: [], dataset: {type: "loot"} }
    };
    ...
    let {items, spells, feats, backgrounds, classes, subclasses} = data.items.reduce((obj, item) => {
      ...
      item.attunement = {
        [CONFIG.EH5E.attunementTypes.REQUIRED]: {
          icon: "fa-sun",
          cls: "not-attuned",
          title: "EH5E.AttunementRequired"
        },
        [CONFIG.EH5E.attunementTypes.ATTUNED]: {
          icon: "fa-sun",
          cls: "attuned",
          title: "EH5E.AttunementAttuned"
        }
    ...
    const maxLevelDelta = CONFIG.EH5E.maxLevel - this.actor.data.data.details.level;
    classes = classes.reduce((arr, cls) => {
      cls.availableLevels = Array.fromRange(CONFIG.EH5E.maxLevel + 1).slice(1).map(level => {
    ...
    for ( const subclass of subclasses ) {
      feats.push(subclass);
      this.actor._preparationWarnings.push(game.i18n.format("EH5E.SubclassMismatchWarn", {
    ...
    const features = {
      background: {
        label: "EH5E.ItemTypeBackground", items: backgrounds,
        hasActions: false, dataset: {type: "background"} },
      classes: {
        label: "EH5E.ItemTypeClassPl", items: classes,
        hasActions: false, dataset: {type: "class"}, isClass: true },
      active: {
        label: "EH5E.FeatureActive", items: [],
        hasActions: true, dataset: {type: "feat", "activation.type": "action"} },
      passive: {
        label: "EH5E.FeaturePassive", items: [],
  ...
  _prepareItemToggleState(item) {
    if (item.type === "spell") {
      ...
      if ( isAlways ) item.toggleTitle = CONFIG.EH5E.spellPreparationModes.always;
      else if ( isPrepared ) item.toggleTitle = CONFIG.EH5E.spellPreparationModes.prepared;
      else item.toggleTitle = game.i18n.localize("EH5E.SpellUnprepared");
    }
    else {
      const isActive = getProperty(item.data, "equipped");
      item.toggleClass = isActive ? "active" : "";
      item.toggleTitle = game.i18n.localize(isActive ? "EH5E.Equipped" : "EH5E.Unequipped");
    }
  }
    ...
    switch ( button.dataset.action ) {
      case "convertCurrency":
        return Dialog.confirm({
          title: `${game.i18n.localize("EH5E.CurrencyConvert")}`,
          content: `<p>${game.i18n.localize("EH5E.CurrencyConvertHint")}</p>`,
  ...
  async _onLevelChange(event) {
    ...
    if ( classItem.hasAdvancement && !game.settings.get("eh5e", "disableAdvancements") ) {
  ...
  async _onDropItemCreate(itemData) {
    ...
    if ( itemData.type === "class" ) {
      itemData.data.levels = Math.min(itemData.data.levels,
        CONFIG.EH5E.maxLevel - this.actor.data.data.details.level);
      if ( itemData.data.levels <= 0 ) return ui.notifications.error(
        game.i18n.format("EH5E.MaxCharacterLevelExceededWarn", {max: CONFIG.EH5E.maxLevel})
      );

      const cls = this.actor.itemTypes.class.find(c => c.identifier === itemData.data.identifier);
      if ( cls ) {
        const priorLevel = cls.data.data.levels;
        if ( cls.hasAdvancement && !game.settings.get("eh5e", "disableAdvancements") ) {
    ...
    // If a subclass is dropped, ensure it doesn't match another subclass with the same identifier
      ...
      if ( other ) {
        return ui.notifications.error(game.i18n.format("EH5E.SubclassDuplicateError", {
          identifier: other.identifier
        }));
      }
      const cls = this.actor.itemTypes.class.find(i => i.identifier === itemData.data.classIdentifier);
      if ( cls && cls.subclass ) {
        return ui.notifications.error(game.i18n.format("EH5E.SubclassAssignmentError", {

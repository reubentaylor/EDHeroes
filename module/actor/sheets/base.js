...
export default class ActorSheet5e extends ActorSheet {
  ...
  static get defaultOptions() {
    return mergeObject(super.defaultOptions, {
      ...
      width: 720,
      height: Math.max(680, 237 + (Object.keys(CONFIG.EH5E.abilities).length * 70))
    });
  }
  ...
  /** @override */
  get template() {
    if ( !game.user.isGM && this.actor.limited ) return "systems/eh5e/templates/actors/limited-sheet.html";
    return `systems/eh5e/templates/actors/${this.actor.data.type}-sheet.html`;
  }
  ...
  getData(options) {
    ...
    // Currency Labels
    data.labels.currencies = Object.entries(CONFIG.EH5E.currencies).reduce((obj, [k, c]) => {
      obj[k] = c.label;
      return obj;
    }, {});
    ...
    // Proficiency
    if ( game.settings.get("eh5e", "proficiencyModifier") === "dice" ) {
    ...
    for ( let [a, abl] of Object.entries(actorData.data.abilities)) {
      ...
      abl.hover = CONFIG.EH5E.proficiencyLevels[abl.proficient];
      abl.label = CONFIG.EH5E.abilities[a];
    ...
    if ( actorData.data.skills ) {
      for (let [s, skl] of Object.entries(actorData.data.skills)) {
        skl.ability = CONFIG.EH5E.abilityAbbreviations[skl.ability];
        skl.icon = this._getProficiencyIcon(skl.value);
        skl.hover = CONFIG.EH5E.proficiencyLevels[skl.value];
        skl.label = CONFIG.EH5E.skills[s];
  ...
  _getMovementSpeed(actorData, largestPrimary=false) {
    ...
    let speeds = [
      [movement.burrow, `${game.i18n.localize("EH5E.MovementBurrow")} ${movement.burrow}`],
      [movement.climb, `${game.i18n.localize("EH5E.MovementClimb")} ${movement.climb}`],
      [movement.fly, `${game.i18n.localize("EH5E.MovementFly")} ${movement.fly}${movement.hover ? ` (${game.i18n.localize("EH5E.MovementHover")})` : ""}`],
      [movement.swim, `${game.i18n.localize("EH5E.MovementSwim")} ${movement.swim}`]
    ];
    if ( largestPrimary ) {
      speeds.push([movement.walk, `${game.i18n.localize("EH5E.MovementWalk")} ${movement.walk}`]);
    }
  ...
  _getSenses(actorData) {
    ...
    const tags = {};
    for ( let [k, label] of Object.entries(CONFIG.EH5E.senses) ) {
      const v = senses[k] ?? 0;
      if ( v === 0 ) continue;
  ...
  _prepareArmorClassAttribution(data) {
    ...
    const cfg = CONFIG.EH5E.armorClasses[ac.calc];
    const attribution = [];
    ...
    switch ( ac.calc ) {
      ...
      case "flat":
        return [{
          label: game.i18n.localize("EH5E.ArmorClassFlat"),
      ...
      case "natural":
        attribution.push({
          label: game.i18n.localize("EH5E.ArmorClassNatural"),
          mode: CONST.ACTIVE_EFFECT_MODES.OVERRIDE,
          value: ac.flat
        });
        break;
      ...
      default:
        ...
        let label = game.i18n.localize("EH5E.PropertyBase");
        if ( armorInFormula ) label = this.actor.armor?.name ?? game.i18n.localize("EH5E.ArmorClassUnarmored");
        attribution.unshift({
          label,
          mode: CONST.ACTIVE_EFFECT_MODES.OVERRIDE,
          value: base
    ...
    if ( ac.shield !== 0 ) attribution.push({
      label: this.actor.shield?.name ?? game.i18n.localize("EH5E.EquipmentShield"),
      mode: CONST.ACTIVE_EFFECT_MODES.ADD,
      value: ac.shield
    });
    ...
    if ( ac.cover !== 0 ) attribution.push({
      label: game.i18n.localize("EH5E.Cover"),
      mode: CONST.ACTIVE_EFFECT_MODES.ADD,
      value: ac.cover
    });
    return attribution;
  ...
  _prepareTraits(traits) {
    const map = {
      dr: CONFIG.EH5E.damageResistanceTypes,
      di: CONFIG.EH5E.damageResistanceTypes,
      dv: CONFIG.EH5E.damageResistanceTypes,
      ci: CONFIG.EH5E.conditionTypes,
      languages: CONFIG.EH5E.languages
    };
  ...
  _prepareSpellbook(data, spells) {
    ...
    if ( maxLevel > 0 ) {
      registerSection("spell0", 0, CONFIG.EH5E.spellLevels[0]);
      for (let lvl = 1; lvl <= maxLevel; lvl++) {
        const sl = `spell${lvl}`;
        registerSection(sl, lvl, CONFIG.EH5E.spellLevels[lvl], levels[sl]);
      }
    }

    // Pact magic users have cantrips and a pact magic section
    if ( levels.pact && levels.pact.max ) {
      if ( !spellbook["0"] ) registerSection("spell0", 0, CONFIG.EH5E.spellLevels[0]);
      const l = levels.pact;
      const config = CONFIG.EH5E.spellPreparationModes.pact;
      const level = game.i18n.localize(`EH5E.SpellLevel${levels.pact.level}`);
    ...
    spells.forEach(spell => {
      ...
      if ( mode in sections ) {
        ...
        if ( !spellbook[s] ) {
          const l = levels[mode] || {};
          const config = CONFIG.EH5E.spellPreparationModes[mode];
      ...
      // Sections for higher-level spells which the caster "should not" have, but spell items exist for
        ...
        registerSection(sl, s, CONFIG.EH5E.spellLevels[s], {levels: levels[sl]});
      }
   ...
   * @param {number} level  A proficiency mode defined in `CONFIG.EH5E.proficiencyLevels`.
  ...
  async _onDropActor(event, data) {
    const canPolymorph = game.user.isGM || (this.actor.isOwner && game.settings.get("eh5e", "allowPolymorphing"));
    ...
    const rememberOptions = html => {
      ...
      const settings = mergeObject(game.settings.get("eh5e", "polymorphSettings") || {}, options);
      game.settings.set("eh5e", "polymorphSettings", settings);
      return settings;
    };

    // Create and render the Dialog
    return new Dialog({
      title: game.i18n.localize("EH5E.PolymorphPromptTitle"),
      content: {
        options: game.settings.get("eh5e", "polymorphSettings"),
        i18n: CONFIG.EH5E.polymorphSettings,
      ...
      buttons: {
        accept: {
          ...
          label: game.i18n.localize("EH5E.PolymorphAcceptSettings"),
          callback: html => this.actor.transformInto(sourceActor, rememberOptions(html))
        },
        wildshape: {
          icon: '<i class="fas fa-paw"></i>',
          label: game.i18n.localize("EH5E.PolymorphWildShape"),
        ...
        polymorph: {
          ...
          label: game.i18n.localize("EH5E.Polymorph"),
          callback: html => this.actor.transformInto(sourceActor, {
            transformTokens: rememberOptions(html).transformTokens
          })
        },
      ...
      classes: ["dialog", "eh5e"],
      width: 600,
      template: "systems/eh5e/templates/apps/polymorph-prompt.html"
    }).render(true);
  }
  ...
  async _onDropItemCreate(itemData) {
    ...
    if ( this.constructor.unsupportedItemTypes.has(itemData.type) ) {
      return ui.notifications.warn(game.i18n.format("EH5E.ActorWarningInvalidItem", {
    ...
    if ( itemData.data.advancement?.length && !game.settings.get("eh5e", "disableAdvancements") ) {
  ...
  _onDropResetData(itemData) {
    ...
    itemData.data.attunement = Math.min(itemData.data.attunement, CONFIG.EH5E.attunementTypes.REQUIRED);
  }
  ...
  _onItemCreate(event) {
    ...
    if ( type === "class" && (this.actor.data.data.details.level + 1 > CONFIG.EH5E.maxLevel) ) {
      return ui.notifications.error(game.i18n.format("EH5E.MaxCharacterLevelExceededWarn",
        {max: CONFIG.EH5E.maxLevel}));
    }

    const itemData = {
      name: game.i18n.format("EH5E.ItemNew", {type: game.i18n.localize(`EH5E.ItemType${type.capitalize()}`)}),
      type: type,
  ...
  async _onItemDelete(event) {
    ...
    if ( item.hasAdvancement && !game.settings.get("eh5e", "disableAdvancements") ) {
  ...
  _onTraitSelector(event) {
    ...
    const a = event.currentTarget;
    const label = a.parentElement.querySelector("label");
    const choices = CONFIG.EH5E[a.dataset.options];
  ...
  _getHeaderButtons() {
    ...
    if ( this.actor.isPolymorphed ) {
      buttons.unshift({
        label: "EH5E.PolymorphRestoreTransformation",

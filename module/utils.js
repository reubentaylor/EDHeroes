...
export function _linkForUuid(uuid) {
  const index = game.eh5e.utils.indexFromUuid(uuid);

  let link;

  if ( !index ) {
    link = `@Item[${uuid}]{${game.i18n.localize("EH5E.Unknown")}}`;
 ...
 * @param {string} configKey              Key within `CONFIG.EH5E` to localize.
 ...
 * @param {object} config  The `CONFIG.EH5E` object to localize and sort. *Will be mutated.*
 */

...
export default class ProficiencySelector extends TraitSelector {
  ...
  static async getChoices(type, chosen=[]) {
    let data = Object.entries(CONFIG.EH5E[`${type}Proficiencies`]).reduce((obj, [key, label]) => {
      obj[key] = { label: label, chosen: chosen.includes(key) };
      return obj;
    }, {});

    const ids = CONFIG.EH5E[`${type}Ids`];
    const map = CONFIG.EH5E[`${type}ProficienciesMap`];
    if ( ids !== undefined ) {
    ...
    if ( type === "tool" ) {
      data.vehicle.children = Object.entries(CONFIG.EH5E.vehicleTypes).reduce((obj, [key, label]) => {
        obj[key] = { label: label, chosen: chosen.includes(key) };
        return obj;
      }, {});
      data = game.eh5e.utils.sortObjectEntries(data, "label");
    }

    for ( const category of Object.values(data) ) {
      if ( !category.children ) continue;
      category.children = game.eh5e.utils.sortObjectEntries(category.children, "label");
    }
  ...
  static getBaseItem(identifier, { indexOnly=false, fullItem=false }={}) {
    let pack = CONFIG.EH5E.sourcePacks.ITEMS;

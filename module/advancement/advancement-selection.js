...
export class AdvancementSelection extends Dialog {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["eh5e", "sheet", "advancement"],
      template: "systems/eh5e/templates/advancement/advancement-selection.html",
      title: "EH5E.AdvancementSelectionTitle",
      width: 500,
      height: "auto"
    });
  }
  ...
  getData() {
    ...
    for ( const advancement of Object.values(game.eh5e.advancement.types) ) {
    ...
    data.types = game.eh5e.utils.sortObjectEntries(data.types, "label");
  ...
  static async createDialog(item, { rejectClose=false, options={} }={}) {
    return new Promise((resolve, reject) => {
      const dialog = new this(item, {
        title: `${game.i18n.localize("EH5E.AdvancementSelectionTitle")}: ${item.name}`,
        buttons: {
          submit: {
            callback: html => {

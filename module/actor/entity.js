...
export default class Actor5e extends Actor {
   ...
   */
  get isPolymorphed() {
    return this.getFlag("eh5e", "isPolymorphed") || false;
  }
  ...
  prepareDerivedData() {
    ...
    const flags = actorData.flags.eh5e || {};
    ...
    if (this.isPolymorphed) {
      const transformOptions = this.getFlag("eh5e", "transformOptions");
      const original = game.actors?.get(this.getFlag("eh5e", "originalActor"));
      if (original) {
    ...
    if ( this.type !== "vehicle" ) data.attributes.attunement.value = this.items.filter(i => {
      return i.data.data.attunement === CONFIG.EH5E.attunementTypes.ATTUNED;
    }).length;
   ...
   */
  getLevelExp(level) {
    const levels = CONFIG.EH5E.CHARACTER_EXP_LEVELS;
  ...
  getCRExp(cr) {
    ...
    return CONFIG.EH5E.CR_EXP_LEVELS[cr];
  }
   ...
   * @deprecated since eh5e 1.6, targeted for removal in 1.8
   */
  async addEmbeddedItems(items, prompt=true) {
    ...
    if (prompt) {
      const itemIdsToAdd = await SelectItemsPrompt.create(items, {
        hint: game.i18n.localize("EH5E.AddEmbeddedItemPromptHint")
      });
  ...
  static async loadClassFeatures({classIdentifier="", subclassName="", level=1, priorLevel=0}={}) {
    ...
    const clsConfig = CONFIG.EH5E.classFeatures[classIdentifier];
    if (!clsConfig) return [];
  ...
  /**
   * Update the actor's abilities list to match the abilities configured in `EH5E.abilities`.
  ...
  _prepareBaseAbilities(actorData, updates) {
    ...
    for ( const key of Object.keys(CONFIG.EH5E.abilities) ) {
  ...
  _prepareSkills(actorData, bonusData, bonuses, checkBonus, originalSkills) {
    ...
    // Skill modifiers
    const feats = CONFIG.EH5E.characterFlags;
    const joat = flags.jackOfAllTrades;
  ...
  _computeSpellcastingProgression(actorData) {
    ...
    const levels = Math.clamped(progression.slot, 0, CONFIG.EH5E.maxLevel);
    const slots = CONFIG.EH5E.SPELL_SLOT_TABLE[Math.min(levels, CONFIG.EH5E.SPELL_SLOT_TABLE.length) - 1] || [];
    ...
    let pl = Math.clamped(progression.pact, 0, CONFIG.EH5E.maxLevel);
  ...
  _computeArmorClass(data) {
    ...
    ac.warnings = [];
    let cfg = CONFIG.EH5E.armorClasses[ac.calc];
    if ( !cfg ) {
      ac.calc = "flat";
      if ( Number.isNumeric(ac.value) ) ac.flat = Number(ac.value);
      cfg = CONFIG.EH5E.armorClasses.flat;
    }

    // Identify Equipped Items
    const armorTypes = new Set(Object.keys(CONFIG.EH5E.armorTypes));
    ...
    switch ( ac.calc ) {
      ...
      default:
        ...
        if ( armors.length ) {
          if ( armors.length > 1 ) ac.warnings.push("EH5E.WarnMultipleArmor");
        ...
        try {
          ...
          ac.warnings.push("EH5E.WarnBadACFormula");
          const replaced = Roll.replaceFormulaData(CONFIG.EH5E.armorClasses.default.formula, rollData);
          ac.base = Roll.safeEval(replaced);
        }
        break;
    }
    ...
    // Equipped Shield
    if ( shields.length ) {
      if ( shields.length > 1 ) ac.warnings.push("EH5E.WarnMultipleShields");
  ...
  _computeEncumbrance(actorData) {
    ...
    if ( game.settings.get("eh5e", "currencyWeight") && actorData.data.currency ) {
      const currency = actorData.data.currency;
      const numCoins = Object.values(currency).reduce((val, denom) => val += Math.max(denom, 0), 0);

      const currencyPerWeight = game.settings.get("eh5e", "metricWeightUnits")
        ? CONFIG.EH5E.encumbrance.currencyPerWeight.metric
        : CONFIG.EH5E.encumbrance.currencyPerWeight.imperial;
    ...
    if ( this.getFlag("eh5e", "powerfulBuild") ) mod = Math.min(mod * 2, 8);

    // Compute Encumbrance percentage
    weight = weight.toNearest(0.1);

    const strengthMultiplier = game.settings.get("eh5e", "metricWeightUnits")
      ? CONFIG.EH5E.encumbrance.strMultiplier.metric
      : CONFIG.EH5E.encumbrance.strMultiplier.imperial;
  ...
  async _preCreate(data, options, user) {
    ...
    // Token size category
    const s = CONFIG.EH5E.tokenSizes[this.data.data.traits.size || "med"];
  ...
  async _preUpdate(changed, options, user) {
    ...
    if ( newSize && (newSize !== foundry.utils.getProperty(this.data, "data.traits.size")) ) {
      let size = CONFIG.EH5E.tokenSizes[newSize];
  ...
  _isRemarkableAthlete(ability) {
    return this.getFlag("eh5e", "remarkableAthlete")
      && CONFIG.EH5E.characterFlags.remarkableAthlete.abilities.includes(ability);
  }
  ...
  rollSkill(skillId, options={}) {
    ...
    const reliableTalent = (skl.value >= 1 && this.getFlag("eh5e", "reliableTalent"));

    // Roll and return
    const flavor = game.i18n.format("EH5E.SkillPromptTitle", {skill: CONFIG.EH5E.skills[skillId]});
    const rollData = foundry.utils.mergeObject(options, {
      ...
      flavor,
      chooseModifier: true,
      halflingLucky: this.getFlag("eh5e", "halflingLucky"),
      reliableTalent: reliableTalent,
      messageData: {
        speaker: options.speaker || ChatMessage.getSpeaker({actor: this}),
        "flags.eh5e.roll": {type: "skill", skillId }
      }
    });
  ...
  rollAbility(abilityId, options={}) {
    const label = CONFIG.EH5E.abilities[abilityId] ?? "";
    new Dialog({
      title: `${game.i18n.format("EH5E.AbilityPromptTitle", {ability: label})}: ${this.name}`,
      content: `<p>${game.i18n.format("EH5E.AbilityPromptText", {ability: label})}</p>`,
      buttons: {
        test: {
          label: game.i18n.localize("EH5E.ActionAbil"),
          callback: () => this.rollAbilityTest(abilityId, options)
        },
        save: {
          label: game.i18n.localize("EH5E.ActionSave"),
  ...
  rollAbilityTest(abilityId, options={}) {
    ...
    // Roll and return
    const flavor = game.i18n.format("EH5E.AbilityPromptTitle", {ability: label});
    const rollData = foundry.utils.mergeObject(options, {
      parts,
      data,
      title: `${flavor}: ${this.name}`,
      ...
      messageData: {
        speaker: options.speaker || ChatMessage.getSpeaker({actor: this}),
        "flags.eh5e.roll": {type: "ability", abilityId }
      }
    });
    return d20Roll(rollData);
  }
  ...
  rollAbilitySave(abilityId, options={}) {
    ...
    // Roll and return
    const flavor = game.i18n.format("EH5E.SavePromptTitle", {ability: label});
    const rollData = foundry.utils.mergeObject(options, {
      parts,
      data,
      title: `${flavor}: ${this.name}`,
      ...
      messageData: {
        speaker: options.speaker || ChatMessage.getSpeaker({actor: this}),
        "flags.eh5e.roll": {type: "save", abilityId }
      }
    });
    return d20Roll(rollData);
  }
  ...
  async rollDeathSave(options={}) {
    ...
    if ( (this.data.data.attributes.hp.value > 0) || (death.failure >= 3) || (death.success >= 3)) {
      ui.notifications.warn(game.i18n.localize("EH5E.DeathSaveUnnecessary"));
      return null;
    }
    ...
    if ( this.getFlag("eh5e", "diamondSoul") ) {
      parts.push("@prof");
    ...
    // Evaluate the roll
    const flavor = game.i18n.localize("EH5E.DeathSavingThrow");
    const rollData = foundry.utils.mergeObject(options, {
      parts,
      data,
      title: `${flavor}: ${this.name}`,
    ...
    if ( success ) {
      ...
      if ( d20 === 20 ) {
        ...
        chatString = "EH5E.DeathSaveCriticalSuccess";
      }
      ...
      // 3 Successes = survive and reset checks
        ...
        chatString = "EH5E.DeathSaveSuccess";
      }
    ...
    // Save failure
      ...
      if ( failures >= 3 ) {  // 3 Failures = death
        chatString = "EH5E.DeathSaveFailure";
      }
    }
  ...
  async rollHitDie(denomination, {dialog=true}={}) {
    ...
    if ( !cls ) {
      ui.notifications.error(game.i18n.format("EH5E.HitDiceWarn", {name: this.name, formula: denomination}));
      return null;
    }

    // Prepare roll data
    const parts = [`1${denomination}`, "@abilities.con.mod"];
    const flavor = game.i18n.localize("EH5E.HitDiceRoll");
   ...
   * @see {@link eh5e.preRollClassHitPoints}
   */
  async rollClassHitPoints(item) {
    ...
    const flavor = game.i18n.format("EH5E.AdvancementHitPointsRollMessage", { class: item.name });
    const messageData = {
      title: `${flavor}: ${this.name}`,
      flavor,
      speaker: ChatMessage.getSpeaker({ actor: this }),
      "flags.eh5e.roll": { type: "hitPoints" }
    };

    /**
     * A hook event that fires before hit points are rolled for a character's class.
     * @function eh5e.preRollClassHitPoints
     * @memberof hookEvents
     ...
     */
    Hooks.callAll("eh5e.preRollClassHitPoints", this, item, rollData, messageData);
  ...
  async _rest(chat, newDay, longRest, dhd=0, dhp=0) {
    ...
    if ( Hooks._hooks.restCompleted?.length ) console.warn(
      "The restCompleted hook has been deprecated in favor of eh5e.restCompleted. "
      + "The original hook will be removed in eh5e 1.8."
    );
     ...
     * @function eh5e.restCompleted
     * @memberof hookEvents
     * @param {Actor5e} actor      The actor that just completed resting.
     * @param {RestResult} result  Details on the rest completed.
     */
    Hooks.callAll("eh5e.restCompleted", this, result);
  ...
  async _displayRestResultMessage(result, longRest=false) {
    ...
    switch (game.settings.get("eh5e", "restVariant")) {
      case "normal": restFlavor = (longRest && newDay) ? "EH5E.LongRestOvernight" : `EH5E.${length}RestNormal`; break;
      case "gritty": restFlavor = (!longRest && newDay) ? "EH5E.ShortRestOvernight" : `EH5E.${length}RestGritty`; break;
      case "epic": restFlavor = `EH5E.${length}RestEpic`; break;
    }

    // Determine the chat message to display
    if ( diceRestored && healthRestored ) message = `EH5E.${length}RestResult`;
    else if ( longRest && !diceRestored && healthRestored ) message = "EH5E.LongRestResultHitPoints";
    else if ( longRest && diceRestored && !healthRestored ) message = "EH5E.LongRestResultHitDice";
    else message = `EH5E.${length}RestResultShort`;
  ...
  convertCurrency() {
    ...
    const conversion = Object.entries(CONFIG.EH5E.currencies);
    conversion.reverse();
  ...
  async transformInto(target, { keepPhysical=false, keepMental=false, keepSaves=false, keepSkills=false,
    ...
    const allowed = game.settings.get("eh5e", "allowPolymorphing");
    if ( !allowed && !game.user.isGM ) {
      return ui.notifications.warn(game.i18n.localize("EH5E.PolymorphWarn"));
    }

    // Get the original Actor data and the new source data
    const o = this.toJSON();
    o.flags.eh5e = o.flags.eh5e || {};
    o.flags.eh5e.transformOptions = {mergeSkills, mergeSaves};
    ...
    if ( !keepClass && d.data.details.cr ) {
      d.items.push({
        type: "class",
        name: game.i18n.localize("EH5E.PolymorphTmpClass"),
    ...
    // Set new data flags
    if ( !this.isPolymorphed || !d.flags.eh5e.originalActor ) d.flags.eh5e.originalActor = this.id;
    d.flags.eh5e.isPolymorphed = true;
     ...
     * @param {TransformationOptions} options  Options that determine how the transformation is performed.
     */
    Hooks.callAll("eh5e.transformActor", this, target, d, {
  ...
  async revertOriginalForm() {
    ...
    if ( !this.isOwner ) {
      return ui.notifications.warn(game.i18n.localize("EH5E.PolymorphRevertWarn"));
    }
  ...
  static addDirectoryContextOptions(html, entryOptions) {
    entryOptions.push({
      name: "EH5E.PolymorphRestoreTransformation",
  ...
  static formatCreatureType(typeData) {
    ...
    if ( typeData.value === "custom" ) {
      ...
      let code = CONFIG.EH5E.creatureTypes[typeData.value];
      localizedType = game.i18n.localize(typeData.swarm ? `${code}Pl` : code);
    }
    let type = localizedType;
    if ( typeData.swarm ) {
      type = game.i18n.format("EH5E.CreatureSwarmPhrase", {
        size: game.i18n.localize(CONFIG.EH5E.actorSizes[typeData.swarm]),
        type: localizedType
      });
    }
  ...
  static prepareProficiencies(data, type) {
    const profs = CONFIG.EH5E[`${type}Proficiencies`];
    const itemTypes = CONFIG.EH5E[`${type}Ids`];
    ...
    for ( const key of values ) {
      ...
      } else if ( type === "tool" && CONFIG.EH5E.vehicleTypes[key] ) {
        data.selected[key] = CONFIG.EH5E.vehicleTypes[key];
      }
    }
  ...
  _displayScrollingDamage(dhp) {
    ...
    for ( let t of tokens ) {
      ...
      t.hud.createScrollingText(dhp.signedString(), {
        ...
        fill: CONFIG.EH5E.tokenHPColors[dhp < 0 ? "damage" : "healing"],
        stroke: 0x000000,
        strokeThickness: 4,
        jitter: 0.25
      });
   ...
   * @param {string} ability  Ability key as defined in `CONFIG.EH5E.abilities`.
   * @returns {number}        Spell save DC for provided ability.
   * @deprecated since eh5e 0.97
   */
  getSpellDC(ability) {

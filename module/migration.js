...
export const migrateWorld = async function() {
  ...
  for ( let a of game.actors ) {
    try {
      ...
      err.message = `Failed eh5e system migration for Actor ${a.name}: ${err.message}`;
      console.error(err);
    }
  }
  ...
  for ( let i of game.items ) {
    try {
      ...
      err.message = `Failed eh5e system migration for Item ${i.name}: ${err.message}`;
      console.error(err);
    }
  }
  ...
  for ( const m of game.macros ) {
    try {
      ...
      err.message = `Failed eh5e system migration for Macro ${m.name}: ${err.message}`;
      console.error(err);
    }
  }
  ...
  for ( let s of game.scenes ) {
    try {
      ...
      err.message = `Failed eh5e system migration for Scene ${s.name}: ${err.message}`;
      console.error(err);
    }
  }
  ...
  game.settings.set("eh5e", "systemMigrationVersion", game.system.data.version);
...
export const migrateCompendium = async function(pack) {
  ...
  for ( let doc of documents ) {
    ...
    // Handle migration failures
      ...
      err.message = `Failed eh5e system migration for document ${doc.name} in pack ${pack.collection}: ${err.message}`;
      console.error(err);
    }
  }
...
export const migrateArmorClass = async function(pack) {
  ...
  const updates = [];
  const armor = new Set(Object.keys(CONFIG.EH5E.armorTypes));
...
function cleanActorData(actorData) {
  ...
  // Scrub system flags
  const allowedFlags = CONFIG.EH5E.allowedActorFlags.reduce((obj, f) => {
    obj[f] = null;
    return obj;
  }, {});
  if ( actorData.flags.eh5e ) {
    actorData.flags.eh5e = filterObject(actorData.flags.eh5e, allowedFlags);
  }
...
export const getMigrationData = async function() {
  const data = {};
  try {
    const icons = await fetch("systems/eh5e/json/icon-migration.json");
    const spellIcons = await fetch("systems/eh5e/json/spell-icon-migration.json");
...
function _migrateActorType(actor, updateData) {
  ...
  if ( match ) {
    ...
    const typeMatch = Object.entries(CONFIG.EH5E.creatureTypes).find(([k, v]) => {
      return (typeLc === k)
    ...
    // Match a swarm
    const isNamedSwarm = actor.name.startsWith(game.i18n.localize("EH5E.CreatureSwarm"));
    if ( match.groups.size || isNamedSwarm ) {
      const sizeLc = match.groups.size ? match.groups.size.trim().toLowerCase() : "tiny";
      const sizeMatch = Object.entries(CONFIG.EH5E.actorSizes).find(([k, v]) => {
...
const TOKEN_IMAGE_RENAME = {
  "systems/eh5e/tokens/beast/OwlWhite.png": "systems/eh5e/tokens/beast/Owl.webp",
  "systems/eh5e/tokens/beast/ScorpionSand.png": "systems/eh5e/tokens/beast/Scorpion.webp",
  "systems/eh5e/tokens/beast/BaboonBlack.png": "systems/eh5e/tokens/beast/Baboon.webp",
  "systems/eh5e/tokens/humanoid/BanditRedM.png": "systems/eh5e/tokens/humanoid/Bandit.webp",
  "systems/eh5e/tokens/humanoid/GuardBlueM.png": "systems/eh5e/tokens/humanoid/Guard.webp",
  "systems/eh5e/tokens/humanoid/HumanBrownM.png": "systems/eh5e/tokens/humanoid/Commoner.webp",
  "systems/eh5e/tokens/humanoid/NobleSwordM.png": "systems/eh5e/tokens/humanoid/Noble.webp",
  "systems/eh5e/tokens/humanoid/MerfolkBlue.png": "systems/eh5e/tokens/humanoid/Merfolk.webp",
  "systems/eh5e/tokens/humanoid/TribalWarriorM.png": "systems/eh5e/tokens/humanoid/TribalWarrior.webp",
  "systems/eh5e/tokens/devil/Lemure.png": "systems/eh5e/tokens/fiend/Lemure.webp",
  "systems/eh5e/tokens/humanoid/Satyr.png": "systems/eh5e/tokens/fey/Satyr.webp",
  "systems/eh5e/tokens/beast/WinterWolf.png": "systems/eh5e/tokens/monstrosity/WinterWolf.webp"
};
...
const TOKEN_IMAGE_RESCALE = {
  "systems/eh5e/tokens/beast/HunterShark.png": 1.5,
  "systems/eh5e/tokens/beast/GiantElk.png": 1.5,
  "systems/eh5e/tokens/monstrosity/Bulette.png": 1.5,
  "systems/eh5e/tokens/beast/Wolf.png": 1.5,
  "systems/eh5e/tokens/beast/Panther.png": 1.5,
  "systems/eh5e/tokens/beast/Elk.png": 1.5,
  "systems/eh5e/tokens/beast/AxeBeak.png": 1.5,
  "systems/eh5e/tokens/beast/GiantVulture.png": 1.5,
  "systems/eh5e/tokens/beast/GiantSpider.png": 1.5,
  "systems/eh5e/tokens/beast/DireWolf.png": 1.5,
  "systems/eh5e/tokens/monstrosity/DeathDog.png": 1.5,
  "systems/eh5e/tokens/devil/Lemure.png": 1.5,
  "systems/eh5e/tokens/beast/Deer.png": 1.1,
  "systems/eh5e/tokens/beast/GiantWeasel.png": 1.5,
  "systems/eh5e/tokens/beast/Camel.png": 1.2,
  "systems/eh5e/tokens/beast/BloodHawk.png": 1.5
};
...
function _migrateTokenImage(actorData, updateData) {
  ["img", "token.img"].forEach(prop => {
    ...
    if ( img === "systems/eh5e/tokens/humanoid/HumanBrownM.webp" ) {
      updateData[prop] = "systems/eh5e/tokens/humanoid/Commoner.webp";
      return updateData;
    }
    if ( !img?.startsWith("systems/eh5e/tokens/") || img?.endsWith(".webp") ) return;
...
function _migrateItemAttunement(item, updateData) {
  ...
  updateData["data.attunement"] = CONFIG.EH5E.attunementTypes.NONE;
...
function _migrateItemRarity(item, updateData) {
  ...
  const rarity = Object.keys(CONFIG.EH5E.itemRarity).find(key =>
    (CONFIG.EH5E.itemRarity[key].toLowerCase() === item.data.rarity.toLowerCase()) || (key === item.data.rarity)
  );
...
function _migrateItemIcon(item, updateData, {iconMap}={}) {
  if ( iconMap && item.img?.startsWith("systems/eh5e/icons/") ) {
...
export async function purgeFlags(pack) {
  const cleanFlags = flags => {
    const flags5e = flags.eh5e || null;
    return flags5e ? {eh5e: flags5e} : {};
  };
  await pack.configure({locked: false});

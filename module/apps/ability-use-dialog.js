...
export default class AbilityUseDialog extends Dialog {
  constructor(item, dialogData={}, options={}) {
    ...
    this.options.classes = ["eh5e", "dialog"];
  ...
  static async create(item) {
    ...
    const data = {
      item: item.data,
      title: game.i18n.format("EH5E.AbilityUseHint", {type: game.i18n.localize(`EH5E.ItemType${item.type.capitalize()}`), name: item.name}),
    ...
    const html = await renderTemplate("systems/eh5e/templates/apps/ability-use.html", data);

    // Create the Dialog and return data as a Promise
    const icon = data.isSpell ? "fa-magic" : "fa-fist-raised";
    const label = game.i18n.localize(`EH5E.AbilityUse${data.isSpell ? "Cast" : "Use"}`);
    return new Promise(resolve => {
      const dlg = new this(item, {
        title: `${item.name}: ${game.i18n.localize("EH5E.AbilityUseConfig")}`,
        content: html,
        buttons: {
          use: {
  ...
  static _getSpellData(actorData, itemData, data) {
    ...
    const consumeSpellSlot = (lvl > 0) && CONFIG.EH5E.spellUpcastModes.includes(itemData.preparation.mode);
    ...
    const spellLevels = Array.fromRange(10).reduce((arr, i) => {
      if ( i < lvl ) return arr;
      const label = CONFIG.EH5E.spellLevels[i];
      ...
      arr.push({
        level: i,
        label: i > 0 ? game.i18n.format("EH5E.SpellLevelSlot", {level: label, n: slots}) : label,
        canCast: max > 0,
        hasSlots: slots > 0
      });
      return arr;
    ...
    if (pact.level >= lvl) {
      spellLevels.push({
        level: "pact",
        label: `${game.i18n.format("EH5E.SpellLevelPact", {level: pact.level, n: pact.value})}`,
        canCast: true,
        hasSlots: pact.value > 0
      });
    }
    const canCast = spellLevels.some(l => l.hasSlots);
    if ( !canCast ) data.errors.push(game.i18n.format("EH5E.SpellCastNoSlots", {
      level: CONFIG.EH5E.spellLevels[lvl],
      name: data.item.name
    }));
  ...
  static _getAbilityUseNote(item, uses, recharge) {
    ...
    if ( quantity <= 0 ) return game.i18n.localize("EH5E.AbilityUseUnavailableHint");

    // Abilities which use Recharge
    if ( recharge.value ) {
      return game.i18n.format(recharge.charged ? "EH5E.AbilityUseChargedHint" : "EH5E.AbilityUseRechargeHint", {
        type: game.i18n.localize(`EH5E.ItemType${item.type.capitalize()}`)
      });
    }
    ...
    if ( item.type === "consumable" ) {
      let str = "EH5E.AbilityUseNormalHint";
      if ( uses.value > 1 ) str = "EH5E.AbilityUseConsumableChargeHint";
      else if ( item.data.quantity === 1 && uses.autoDestroy ) str = "EH5E.AbilityUseConsumableDestroyHint";
      else if ( item.data.quantity > 1 ) str = "EH5E.AbilityUseConsumableQuantityHint";
      return game.i18n.format(str, {
        type: game.i18n.localize(`EH5E.Consumable${item.data.consumableType.capitalize()}`),
        value: uses.value,
        quantity: item.data.quantity,
        max: uses.max,
        per: CONFIG.EH5E.limitedUsePeriods[uses.per]
      });
    }

    // Other Items
    else {
      return game.i18n.format("EH5E.AbilityUseNormalHint", {
        type: game.i18n.localize(`EH5E.ItemType${item.type.capitalize()}`),
        value: uses.value,
        max: uses.max,
        per: CONFIG.EH5E.limitedUsePeriods[uses.per]

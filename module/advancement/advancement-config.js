...
export class AdvancementConfig extends FormApplication {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["eh5e", "advancement", "dialog"],
      template: "systems/eh5e/templates/advancement/advancement-config.html",
      width: 400,
      height: "auto",
      submitOnChange: true,
      closeOnSubmit: false
  ...
  get title() {
    ...
    return `${game.i18n.format("EH5E.AdvancementConfigureTitle", { item: this.item.name })}: ${type}`;
  }
  ...
  /** @inheritdoc */
  getData() {
    const levels = Object.fromEntries(Array.fromRange(CONFIG.EH5E.maxLevel + 1).map(l => [l, l]));
    if ( ["class", "subclass"].includes(this.item.type) ) delete levels[0];
    else levels[0] = game.i18n.localize("EH5E.AdvancementLevelAnyHeader");

...
export class ScaleValueAdvancement extends Advancement {
  ...
  static get metadata() {
    return foundry.utils.mergeObject(super.metadata, {
      ...
      order: 60,
      icon: "systems/eh5e/icons/svg/scale-value.svg",
      title: game.i18n.localize("EH5E.AdvancementScaleValueTitle"),
      hint: game.i18n.localize("EH5E.AdvancementScaleValueHint"),
      multiLevel: true,
   ...
   */
  static TYPES = {
    string: "EH5E.AdvancementScaleValueTypeString",
    number: "EH5E.AdvancementScaleValueTypeNumber",
    dice: "EH5E.AdvancementScaleValueTypeDice",
    distance: "EH5E.AdvancementScaleValueTypeDistance"
  };
  ...
  formatValue(level) {
    ...
    const value = this.valueForLevel(level);
    if ( value == null ) return null;
    return `${value.value} ${CONFIG.EH5E.movementUnits[this.data.configuration.distance.units]}`;
  }
...
export class ScaleValueConfig extends AdvancementConfig {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["eh5e", "advancement", "scale-value", "two-column"],
      template: "systems/eh5e/templates/advancement/scale-value-config.html",
      width: 540
    });
  }
  ...
  getData() {
    ...
    data.typeHint = game.i18n.localize(`EH5E.AdvancementScaleValueTypeHint${config.type.capitalize()}`);
    data.types =
      Object.fromEntries(
    ...
    data.movementUnits = CONFIG.EH5E.movementUnits;
    return data;
  }
  ...
  _prepareLevelData() {
    let lastValue = null;
    return Array.fromRange(CONFIG.EH5E.maxLevel + 1).slice(1).reduce((obj, level) => {
  ...
  async _updateObject(event, formData) {
    ...
    if ( typeChange && (formData["data.configuration.type"] !== this.advancement.data.configuration.type) ) {
      ...
      Array.fromRange(CONFIG.EH5E.maxLevel + 1).slice(1).forEach(l =>
...
export class ScaleValueFlow extends AdvancementFlow {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      template: "systems/eh5e/templates/advancement/scale-value-flow.html"
    });
  }

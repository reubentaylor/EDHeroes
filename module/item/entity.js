...
export default class Item5e extends Item {
  ...
  get abilityMod() {
    ...
    // Case 2 - inferred from a parent actor
      ...
      // Weapons
        ...
        else if ( ["basicR", "advancedR", "militaryR"].includes(wt) ) return "dex";
      }
  ...
  get hasAreaTarget() {
    const target = this.data.data.target;
    return target && (target.type in CONFIG.EH5E.areaTargetTypes);
  }
   ...
   */
  get isArmor() {
    return this.data.data.armor?.type in CONFIG.EH5E.armorTypes;
  }
  ...
  get areEffectsSuppressed() {
    ...
    return this.data.data.attunement === CONFIG.EH5E.attunementTypes.REQUIRED;
  }
  ...
  prepareDerivedData() {
    ...
    // Feat Items
      ...
      if ( act && (act.type === C.abilityActivationTypes.legendary) ) labels.featType = game.i18n.localize("EH5E.LegendaryActionLabel");
      else if ( act && (act.type === C.abilityActivationTypes.lair) ) labels.featType = game.i18n.localize("EH5E.LairActionLabel");
      else if ( act && act.type ) labels.featType = game.i18n.localize(data.damage.length ? "EH5E.Attack" : "EH5E.Action");
      else labels.featType = game.i18n.localize("EH5E.Passive");
    }

    // Equipment Items
    else if ( itemData.type === "equipment" ) {
      labels.armor = data.armor.value ? `${data.armor.value} ${game.i18n.localize("EH5E.AC")}` : "";
    }
    ...
    if ( data.hasOwnProperty("activation") ) {
      ...
      labels.recharge = `${game.i18n.localize("EH5E.Recharge")} [${chg.value}${parseInt(chg.value) < 6 ? "+" : ""}]`;
    }
  ...
  _prepareAdvancement() {
    ...
    this.advancement = {
      ...
      byLevel: Object.fromEntries(
        Array.fromRange(CONFIG.EH5E.maxLevel + 1).slice(minAdvancementLevel).map(l => [l, []])
      ),
      byType: {},
      needingConfiguration: []
    };
    for ( const advancementData of this.data.data.advancement ?? [] ) {
      const Advancement = game.eh5e.advancement.types[`${advancementData.type}Advancement`];
  ...
  prepareFinalAttributes() {
    ...
    if ( this.data.data.hasOwnProperty("actionType") ) {
      // Ability checks
      this.labels.abilityCheck = game.i18n.format("EH5E.AbilityPromptTitle", {
        ability: CONFIG.EH5E.abilities[this.data.data?.ability]
      });
  ...
  getDerivedDamageLabel() {
    ...
    const damageLabels = { ...CONFIG.EH5E.damageTypes, ...CONFIG.EH5E.healingTypes };
  ...
  getSaveDC() {
    ...
    // Update labels
    const abl = CONFIG.EH5E.abilities[save.ability] ?? "";
    this.labels.save = game.i18n.format("EH5E.SaveDC", {dc: save.dc || "", ability: abl});
    return save.dc;
  }
  ...
  getCriticalThreshold() {
    const itemData = this.data.data;
    const actorFlags = this.actor.data.flags.eh5e || {};
    if ( !this.hasAttack || !itemData ) return;
  ...
  async roll({configureDialog=true, rollMode, createMessage=true}={}) {
    ...
    const requireSpellSlot = isSpell && (id.level > 0) && CONFIG.EH5E.spellUpcastModes.includes(id.preparation.mode);
    ...
    if ( createMeasuredTemplate ) {
      const template = game.eh5e.canvas.AbilityTemplate.fromItem(item);
  ...
  _getUsageUpdates({consumeQuantity, consumeRecharge, consumeResource, consumeSpellLevel, consumeUsage}) {
    ...
    if ( consumeRecharge ) {
      ...
      if ( recharge.charged === false ) {
        ui.notifications.warn(game.i18n.format("EH5E.ItemNoUses", {name: this.name}));
        return false;
      }
    ...
    if ( consumeSpellLevel ) {
      ...
      if ( spells === 0 ) {
        const label = game.i18n.localize(consumeSpellLevel === "pact" ? "EH5E.SpellProgPact" : `EH5E.SpellLevel${id.level}`);
        ui.notifications.warn(game.i18n.format("EH5E.SpellCastNoSlots", {name: this.name, level: label}));
        return false;
      }
  ...
  _handleConsumeResource(itemUpdates, actorUpdates, resourceUpdates) {
    ...
    // No consumed target
    const typeLabel = CONFIG.EH5E.abilityConsumptionTypes[consume.type];
    if ( !consume.target ) {
      ui.notifications.warn(game.i18n.format("EH5E.ConsumeWarningNoResource", {name: this.name, type: typeLabel}));
      return false;
    }
    ...
    if ( resource === undefined ) {
      ui.notifications.warn(game.i18n.format("EH5E.ConsumeWarningNoSource", {name: this.name, type: typeLabel}));
      return false;
    }
    ...
    if ( remaining < 0 ) {
      ui.notifications.warn(game.i18n.format("EH5E.ConsumeWarningNoQuantity", {name: this.name, type: typeLabel}));
      return false;
    }
  ...
  async displayCard({rollMode, createMessage=true}={}) {
    ...
    const html = await renderTemplate("systems/eh5e/templates/chat/item-card.html", templateData);
    ...
    if ( (this.data.type === "consumable") && !this.actor.items.has(this.id) ) {
      chatData.flags["eh5e.itemData"] = this.data;
    }
  ...
  getChatData(htmlOptions={}) {
    ...
    if ( data.hasOwnProperty("equipped") && !["loot", "tool"].includes(this.data.type) ) {
      if ( data.attunement === CONFIG.EH5E.attunementTypes.REQUIRED ) {
        props.push(CONFIG.EH5E.attunements[CONFIG.EH5E.attunementTypes.REQUIRED]);
      }
      props.push(
        game.i18n.localize(data.equipped ? "EH5E.Equipped" : "EH5E.Unequipped"),
        game.i18n.localize(data.proficient ? "EH5E.Proficient" : "EH5E.NotProficient")
      );
    }
  ...
  _equipmentChatData(data, labels, props) {
    props.push(
      CONFIG.EH5E.equipmentTypes[data.armor.type],
      labels.armor || null,
      data.stealth.value ? game.i18n.localize("EH5E.StealthDisadvantage") : null
    );
  }
  ...
  _weaponChatData(data, labels, props) {
    props.push(
      CONFIG.EH5E.weaponTypes[data.weaponType]
    );
  }
  ...
  _consumableChatData(data, labels, props) {
    props.push(
      CONFIG.EH5E.consumableTypes[data.consumableType],
      `${data.uses.value}/${data.uses.max} ${game.i18n.localize("EH5E.Charges")}`
    );
  ...
  _toolChatData(data, labels, props) {
    props.push(
      CONFIG.EH5E.abilities[data.ability] || null,
      CONFIG.EH5E.proficiencyLevels[data.proficient || 0]
    );
  }
  ...
  _lootChatData(data, labels, props) {
    props.push(
      game.i18n.localize("EH5E.ItemTypeLoot"),
      data.weight ? `${data.weight} ${game.i18n.localize("EH5E.AbbreviationLbs")}` : null
    );
  }
  ...
  async rollAttack(options={}) {
    const itemData = this.data.data;
    const flags = this.actor.data.flags.eh5e || {};
    if ( !this.hasAttack ) {
      throw new Error("You may not place an Attack Roll with this Item.");
    }
    let title = `${this.name} - ${game.i18n.localize("EH5E.AttackRoll")}`;
    ...
    let rollConfig = {
      ...
      messageData: {
        "flags.eh5e.roll": {type: "attack", itemId: this.id },
  ...
  rollDamage({critical=false, event=null, spellLevel=null, versatile=false, options={}}={}) {
    ...
    const actorData = this.actor.data.data;
    const messageData = {
      "flags.eh5e.roll": {type: "damage", itemId: this.id},
      speaker: ChatMessage.getSpeaker({actor: this.actor})
    };
    ...
    const actionFlavor = game.i18n.localize(itemData.actionType === "heal" ? "EH5E.Healing" : "EH5E.DamageRoll");
    ...
    if ( versatile && itemData.damage.versatile ) {
      ...
      messageData["flags.eh5e.roll"].versatile = true;
    }
    ...
    if ( itemData.actionType === "mwak" ) {
      rollConfig.criticalBonusDice = this.actor.getFlag("eh5e", "meleeCriticalDamageDice") ?? 0;
    }
  ...
  async rollFormula({spellLevel}={}) {
    ...
    const rollData = this.getRollData();
    if ( spellLevel ) rollData.item.level = spellLevel;
    const title = `${this.name} - ${game.i18n.localize("EH5E.OtherFormula")}`;
    ...
    roll.toMessage({
      ...
      messageData: {"flags.eh5e.roll": {type: "other", itemId: this.id }}
    });
    return roll;
  }
  ...
  async rollRecharge() {
    ...
    const promises = [roll.toMessage({
      flavor: `${game.i18n.format("EH5E.ItemRechargeCheck", {name: this.name})} - ${game.i18n.localize(success ? "EH5E.ItemRechargeSuccess" : "EH5E.ItemRechargeFailure")}`,
  ...
  rollToolCheck(options={}) {
    ...
    const title = `${this.name} - ${game.i18n.localize("EH5E.ToolCheck")}`;
    ...
    const rollConfig = mergeObject({
      ...
      chooseModifier: true,
      halflingLucky: this.actor.getFlag("eh5e", "halflingLucky" ) || false,
      reliableTalent: (this.data.data.proficient >= 1) && this.actor.getFlag("eh5e", "reliableTalent"),
      messageData: {
        speaker: options.speaker || ChatMessage.getSpeaker({actor: this.actor}),
        "flags.eh5e.roll": {type: "tool", itemId: this.id }
      }
    }, options);
  ...
  static async _onChatCardAction(event) {
    ...
    const storedData = message.getFlag("eh5e", "itemData");
    const item = storedData ? new this(storedData, {parent: actor}) : actor.items.get(card.dataset.itemId);
    if ( !item ) {
      return ui.notifications.error(game.i18n.format("EH5E.ActionWarningNoItem", {item: card.dataset.itemId, name: actor.name}));
    }
  ...
  static _getChatCardTargets(card) {
    ...
    if ( !targets.length ) ui.notifications.warn(game.i18n.localize("EH5E.ActionWarningNoToken"));
    return targets;
  }
  ...
  async createAdvancement(type, data={}, { showConfig=true }={}) {
    ...
    const Advancement = game.eh5e.advancement.types[`${type}Advancement`];
    if ( !Advancement ) throw new Error(`${type}Advancement not found in game.eh5e.advancement.types`);
  ...
  async _preUpdate(changed, options, user) {
    ...
    if ( changed.data.levels <= 0 ) {
      ui.notifications.warn(game.i18n.localize("EH5E.MaxClassLevelMinimumWarn"));
      changed.data.levels = 1;
    }

    // Check to make sure the updated class level doesn't exceed level cap
    if ( changed.data.levels > CONFIG.EH5E.maxLevel ) {
      ui.notifications.warn(game.i18n.format("EH5E.MaxClassLevelExceededWarn", {max: CONFIG.EH5E.maxLevel}));
      changed.data.levels = CONFIG.EH5E.maxLevel;
    }
    ...
    if ( newCharacterLevel > CONFIG.EH5E.maxLevel ) {
      ui.notifications.warn(game.i18n.format("EH5E.MaxCharacterLevelExceededWarn",
        {max: CONFIG.EH5E.maxLevel}));
      changed.data.levels -= newCharacterLevel - CONFIG.EH5E.maxLevel;
    }
  }
  ...
  _onCreateOwnedEquipment(data, actorData, isNPC) {
    ...
    if ( foundry.utils.getProperty(data, "data.proficient") === undefined ) {
      if ( isNPC ) {
        ...
        const armorProf = CONFIG.EH5E.armorProficienciesMap[this.data.data.armor?.type]; // Player characters check proficiency
  ...
  _onCreateOwnedWeapon(data, actorData, isNPC) {
    ...
    const weaponProf = CONFIG.EH5E.weaponProficienciesMap[this.data.data.weaponType];
  ...
  static async createScrollFromSpell(spell) {
    ...
    // Get scroll data
    const scrollUuid = `Compendium.${CONFIG.EH5E.sourcePacks.ITEMS}.${CONFIG.EH5E.spellScrollIds[level]}`;
    ...
    const spellScrollData = foundry.utils.mergeObject(scrollData, {
      name: `${game.i18n.localize("EH5E.SpellScroll")}: ${itemData.name}`,
      img: itemData.img,
      data: {
